...
def main(args=None, input=None, output=None):
    ...
    default_input_history_file = (
        os.path.join(
            git_root, ".aider.input.history") if git_root else ".aider.input.history"
    )
    default_chat_history_file = (
        os.path.join(
            git_root, ".aider.chat.history.md") if git_root else ".aider.chat.history.md"
    )
    ...
    if not args.openai_api_key:
        ...
        if os.name == "nt":
            ...
            # )
            print("No OPEN-AI-Key on Winddows")
        else:
            # io.tool_error(
